...
CAgoraObject* CAgoraObject::getInstance(QObject *parent, QString strId)
{
    std::lock_guard<std::mutex> autoLock(m_mutex);
    if(nullptr == m_pInstance)
        m_pInstance = new CAgoraObject(parent, strId);
...
CAgoraObject::CAgoraObject(QObject *parent, QString strId):
    QObject(parent),
    ...
    m_eventHandler(new AgoraRtcEngineEvent(*this))
{
	...
	std::string stdStrId = strId.toStdString();
    context.appId = stdStrId.c_str();
    if (*context.appId == '\0')
    {
